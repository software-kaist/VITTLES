...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id ||
     ...
     */
    __android_log_print (ANDROID_LOG_ERROR, "PlayActivity", "The calling class does not implement all necessary interface methods");
    return JNI_FALSE;
  }
  return JNI_TRUE;
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if ((*vm)->GetEnv(vm, (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
    __android_log_print (ANDROID_LOG_ERROR, "PlayActivity", "Could not retrieve JNIEnv");
    return 0;
  }
  jclass klass = (*env)->FindClass (env, "kaist/game/battlecar/PlayActivity");
