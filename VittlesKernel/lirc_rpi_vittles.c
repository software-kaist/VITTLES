...
#define LIRC_DRIVER_NAME "lirc_rpi_vittles"
#define RBUF_LEN 256
...
/* set the default GPIO input pins */
#define INPUT_PIN_NUM 4
static int gpio_in_pin[INPUT_PIN_NUM] = {12,16,20,21};
/* set the default IR check valure */
static int ir_check_val =0;
/* set the default pull behaviour for input pin */
static int gpio_in_pull = BCM2708_PULL_DOWN;
/* set the default GPIO output pin */
static int gpio_out_pin = 26;
...
static int irq_num[INPUT_PIN_NUM];
...
static struct timeval lasttv[INPUT_PIN_NUM] = {{0,0},{0,0},{0,0},{0,0}};
...
static ssize_t show_hit(struct device *dev,	
					struct device_attribute *attr, char *buf);
static ssize_t store_timefor_shoot(struct device *dev, 
		struct device_attribute *attr, const char *buf, size_t count);
	
static DEVICE_ATTR(shoot, S_IRUGO|S_IWUSR, NULL, store_timefor_shoot);
static DEVICE_ATTR(hit, S_IRUGO, show_hit , NULL);
...
static struct attribute *vittles_attributes[] ={
	&dev_attr_shoot.attr,
	&dev_attr_hit.attr,
	NULL
};

static const struct attribute_group vittles_group = {
	.attrs = vittles_attributes,
};
static const struct attribute_group *vittles_groups[] = {
	&vittles_group,
	NULL
};
...
static const struct of_device_id lirc_rpi_of_match[] = {
	{ .compatible = "rpi,lirc-rpi-vittles", },
	{},
};
...
module_param_array(gpio_in_pin, int, NULL, S_IRUGO);
MODULE_PARM_DESC(gpio_in_pin, "GPIO input pin number of the BCM processor."
		 " (default 18");

module_param(ir_check_val, int, S_IRUGO);
MODULE_PARM_DESC(ir_check_val, "vittles shoot check(0x00000000 F-B-L-R)");
...
module_param(softcarrier, bool, S_IRUGO | S_IWUSR);
...
module_param(length, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(debug, "choose length for pulse ");
